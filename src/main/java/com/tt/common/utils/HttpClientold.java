...
public class HttpClientold {
    private Logger log = LoggerFactory.getLogger(HttpClientold.class);
    ...
    private HttpClientold(Builder builder){
    ...
    public String proxyGet(String url,String charset, Integer connTimeout, Integer readTimeout, Integer retry, Map<String, String> headers ) {
        return fnBody(retry, headers, new Function<HttpHostExt, String>() {
            ...
            public String apply(HttpHostExt httpHostExt) {
                return httpRequest(url, GET,charset,connTimeout, readTimeout, headers,null,httpHostExt.getHttpHost())+"";
            }
        });
    ...
    public String proxyPostFrom(String url,String charset, Integer connTimeout, Integer readTimeout, Integer retry, Map<String, String> headers,Map<String, String> params){
        return fnBody(retry, headers, new Function<HttpHostExt, String>() {
            @Override
            public String apply(HttpHostExt httpHostExt) {
                return  httpRequest(url, POST_FROM ,charset,connTimeout, readTimeout, headers,params,httpHostExt.getHttpHost());
            }
        });
    }
    ...
    public String proxyPostJson(String url, String charset, Integer connTimeout, Integer readTimeout, Integer retry, Map<String, String> headers, String json) {
        return fnBody(retry, headers, new Function<HttpHostExt, String>() {
            @Override
            public String apply(HttpHostExt httpHostExt) {
                return  httpRequest(url,POST_JSON,charset,connTimeout, readTimeout, headers,json,httpHostExt.getHttpHost());
            }
        });
    }
    ...
    private String fnBody(Integer retry, Map<String, String> headers, Function<HttpHostExt,String> fn){
        ...
        for (int i = 0; i<=cnt && flag; i++){
            ...
            headers.putAll(httpHostExt.getHeaders());
    ...
    private HttpHostExt getHttpHost(){
        ...
        do{
            httpHostExt.setHttpHost(IP_LIST.poll());
            //请求代理池获取ip
            boolean def = true;
            if(StringUtils.isNotBlank(this.proxyUrl)&&IP_LIST.size()<=proxyLessThan){
                try {
                    LOCK.lock();
                    if(IP_LIST.size()<=proxyLessThan){
                        ...
                        if(StringUtils.isNotBlank(body)&&body.contains(":")){
                            ...
                            for(int y=1 ;y<ips.length; y++){
                                ipAndPort = ips[y].split(":");
                                IP_LIST.offer(new HttpHost(ipAndPort[0],Integer.valueOf(ipAndPort[1])));
                            }
                            def = false;
                        }else {
                            def = true;
        ...
        }while (httpHostExt.getHttpHost()==null);
        return httpHostExt;
    }
    ...
    //host扩展
    private static class HttpHostExt{
        HttpHost httpHost;
    ...
    public static final class Builder {
        ...
        public Builder(){
            ...
            this.proxyRetryTag.add("ejuResponseCode=502");
        }
        ...
        public Builder proxyJsonDefault(String proxyJsonDefault){
            if(this.proxyJsonDefault ==null){
                throw new NullPointerException("proxyJsonDefault == null");
            }else if(proxyJsonDefault.indexOf("hostname")==-1
        ...
        public HttpClientold builder(){
            return new HttpClientold(this);
        }
    ...
    public static void main(String[] args) {
        ...
        **/
        ...
        HttpClientold httpClient = HttpClientold.newBuilder()
        ...
        for (int i=0; i< 5 ; i++){
            new Thread(new Runnable() {
                @Override
                public void run() {
                       String html = httpClient.proxyGet("http://www.fang.com/SoufunFamily.htm","utf-8",2000,2000, 2,header);
            ...
            new Thread(new Runnable() {
                @Override
                public void run() {
                    String html = httpClient.proxyGet("https://hz.lianjia.com/ershoufang/103102678884.html","utf-8",2000,2000, 2,header);
                    System.out.println(header.get("responseHeaders"));
                }
            }).start();
        }
